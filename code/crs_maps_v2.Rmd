...
```{r data}
realm_ctry_info <- readRDS("/scratch/network/rsenior/hmei-internship/realm_ctry_info.Rds")
load("/scratch/network/rsenior/hmei-internship/msdata.Rdata")
...
trait_df <- 
  ...
  filter(!(is.na(NES)),
         !(is.na(diff_top25))) %>% 
  dplyr::select(scientificName, orderName, className, familyName,
                redlistCategory, areaAOH, 
                Category, Difference, NES, diff_top25)
...
country_polys <- 
  read_sf("/scratch/network/rsenior/conservation-actions/spatial/gadm36_0.shp") %>% 
...
crs_realm_total <-
  ...
  filter(NES == 1) %>%
  bind_rows() %>% 
  left_join(readRDS("/scratch/network/rsenior/hmei-internship/realm_ctry_info.Rds"), 
            by = "scientificName") %>% 
  filter(!(is.na(realms))) %>% 
...
top25_realm_total_combi <- 
  ...
  group_by(realm) %>% 
  summarise(n = n()) %>% 
  mutate(value = "(b) Extreme No Exit Species")

# Join to spatial data
top25_realm_total_combi <- 
  left_join(realm_polys, top25_realm_total_combi, by = c("Realm" = "realm"))
```
...
# Join to spatial data
top25_country_total_combi <-
    left_join(country_polys, top25_country_total_combi, by = c("NAME_0" = "country")) %>% 
  filter(!(is.na(value)))
```
...
ggsave(plot = map_CRS_realm_combi,
       filename = "/scratch/network/rsenior/hmei-internship/figs/nes_map_realm.png",
...
ggsave(plot = map_CRS_ctry_combi,
       filename = "/scratch/network/rsenior/hmei-internship/figs/nes_map_country.png",
       width = 7, height = 20, units = "cm", dpi = 800)
```

# Map top 25%

```{r map-top25-raw, eval = FALSE}
# Make maps!
map_top25 <-
...
map_top25_realm <-
  ...
  lapply(top25_realm_total_polys, function(taxon){
    ggplot(taxon) +
      geom_sf(data = realm_polys, 
              colour = "transparent", 
              fill = "grey") +
...
ggsave(plot = map_top25_realm_combi,
       filename = "/scratch/network/rsenior/hmei-internship/figs/top25_map_realm.png",
       width = 7, height = 20, units = "cm", dpi = 800)
```
...
map_top25_ctry <-
  lapply(top25_country_total_polys, function(taxon){
    ggplot(taxon) +
      geom_sf(data = realm_polys, 
              colour = "transparent", 
              fill = "grey") +
...
ggsave(plot = map_top25_ctry_combi,
       filename = "/scratch/network/rsenior/hmei-internship/figs/top25_map_country.png",
       width = 7, height = 20, units = "cm", dpi = 800)
```
...
top25_realm_p <-
  ...
  geom_sf(data = realm_polys, 
          colour = "transparent", 
          fill = "grey") +
  geom_sf(data = top25_realm_total_combi,
          aes(fill = n),
          colour = "transparent") +
  facet_wrap(~ value) +
  theme_void() +
...
map_realm_combi <- 
  cowplot::plot_grid(crs_realm_p,top25_realm_p, ncol = 1)
ggsave(plot = map_realm_combi,
       filename = "/scratch/network/rsenior/hmei-internship/figs/map_realm_combi.png",
...
top25_country_p <-
  ...
  geom_sf(data = country_polys, 
          colour = "transparent", 
          fill = "grey") +
  geom_sf(data = top25_country_total_combi,
          aes(fill = n),
          colour = "transparent") +
  facet_wrap(~ value) +
  theme_void() +
...
map_country_combi <- 
  cowplot::plot_grid(crs_country_p,top25_country_p, ncol = 1)
ggsave(plot = map_country_combi,
       filename = "/scratch/network/rsenior/hmei-internship/figs/map_country_combi.png",
       width = 12, height = 14, units = "cm", dpi = 800)
```
