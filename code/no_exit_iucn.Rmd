...
files <- 
  lapply(c("amphibia", "aves", "magnoliopsida", "mammalia", "reptilia"),
         function(x){
           list.files(path = file.path("D:/Work/Data/GEE/jung2015_RL", 
                                       paste("jung2015_RL", x, sep = "_")), 
...
```{r merge-data}
# read iucn data
iucndat <- read_csv('HMEI-GDrive/data/iucn_dat.csv') %>% 
...
merge_dat <- 
  ...
  dplyr::select(species,className,areaAOH,realm,country,redlistCategory)
```
...
# write.csv(test,"HMEI-GDrive/data/nes.csv",fileEncoding = "UTF8")

# graphs
ggplot(merge_dat, aes(x=className, fill=className)) + 
  geom_bar() + 
  facet_wrap(~Category) + 
...
```{r sort-realms}
sort(unique(unlist(strsplit(unique(merge_dat$realm), "[|]"))))
...
realms_long <- 
  ...
  pivot_longer(cols=c(Afrotropical, Australasian, Indomalayan, Nearctic, 
                      Neotropical, Oceanian, Palearctic), 
...
ggplot(realms_long, aes(x=Realms)) + 
  geom_bar() + 
  theme(axis.text.x = element_text(angle = 90)) + 
  ggtitle("Number of CRS in each Realm")

ggplot(realms_long, aes(x=Realms, fill=Realms)) + 
  geom_bar() + 
  facet_wrap(~Category) + 
  theme(axis.text.x = element_blank()) + 
...
ggplot(merge_dat, aes(x=Difference, fill=className, alpha=0.01)) + 
  geom_histogram() + 
...
ggplot(merge_dat, aes(x=Difference)) + 
  geom_histogram() + 
...
```{r diff-hist-class}
ggplot(merge_dat, aes(x=Difference, fill=className)) + 
...
ggplot(merge_dat, aes(x=Difference, fill=Category, alpha=0.5)) + 
  geom_histogram() + 
...
Realm
```{r diff-hist-realm}
ggplot(realms_long, aes(x=Difference, fill=Realms)) + 
  geom_histogram() + 
  ggtitle("Distribution of Differences Across Realm")

ggplot(realms_long, aes(x=Difference)) + 
  geom_histogram() + 
  facet_wrap(~Realms)
...
crit_top = merge_dat %>% filter(Category == 'Critically Endangered') %>% filter(Difference >= 8.754866)

endan_top = merge_dat %>% filter(Category == 'Endangered') %>% filter(Difference >= 459.977142)

vul_top = merge_dat %>% filter(Category == 'Vulnerable') %>% filter(Difference >= 1278.32419)
...
```{r nes-count-plot}
ggplot(merge_dat, aes(x = as.factor(NES))) +
    geom_bar() +
...
nes_difference <- nes %>% 
  mutate(difference_prop = ifelse(Category == "Critically Endangered", Difference / 10, 
                                  ifelse(Category == "Endangered", Difference / 491, 
                                         ifelse(Category == "Vulnerable", Difference / 1501, NA))),
...
nes_top25 <- data %>% mutate(CRS = as.numeric(AOHkm < 1999)) %>% mutate(top25 = ifelse(Category == 'Critically Endangered' & Difference >= 8.754866, 1, ifelse(Category == 'Endangered' & Difference >= 459.977142, 1, ifelse(Category == 'Vulnerable' & Difference >= 1278.32419, 1, 0))))

nes_top25 <- nes_top25 %>% rename(scientificName = species) 

write.csv(nes_top25,"HMEI-GDrive/data/nes_top25.csv",fileEncoding = "UTF8")
...
prop <- nes_top25 %>%
  ...
  dplyr::select(scientificName, phylumName, orderName, className, familyName,
                ...
                Category, Difference, CRS, top25, realm, country)
...
prop_test <- prop %>%
  ...
  group_by(realm, className, CRS) %>% 
  summarise(n = n())


prop1 = prop_test %>% filter(CRS == 1)
prop2 = prop %>%
  filter(!(is.na(realm))) %>% 
  # Separate - species occurs in multiple rows if range across multiple realms
  separate_rows(realm, sep = "\\|") %>% 
  ...
  summarise(total = n())
prop3 = inner_join(prop2, prop1, by=c('realm'='realm', 'className' = 'className'))
prop3 = prop3 %>% mutate(prop = n / total)
