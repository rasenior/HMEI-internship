...
change2 <- test %>% filter(cat_red_change == -2) %>% select(scientificName, className)
```
...
```{r}
df %>% group_by(Category, realm, className) %>% summarize(Sum = n()) %>% arrange(-Sum)

df %>% group_by(redlistCategory, realm, className) %>% summarize(Sum = n()) %>% arrange(-Sum)

df %>% distinct(redlistCategory)
