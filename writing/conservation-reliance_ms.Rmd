...
nes_g <- read_csv('nes.csv')

nes_g <- nes_g %>% mutate(Category = str_replace(Category, "Critically Endangered", "AOH < 10 km^2")) %>% 
...
# relevel Categories
nes_g$Category <- factor(nes_g$Category, levels = c("AOH < 10 km^2", "10 < AOH < 500 km^2", "500 < AOH < 2000 km^2"))

max_val <- 834

ggplot(nes_g, aes(x=className, fill=className)) + 
  geom_bar() + 
...
ggplot(nes_g, aes(x=Difference, fill=className)) + geom_histogram(alpha=0.7) + 
