...
Across class and Red List thresholds, there is a right skew towards a large difference between species' AOH and the downlisting threshold, particular for those species with AOH between 10 km^2 and 500 km^2 (Figure \@ref(fig:difference-histogram)). This suggests that, within each range of AOH values (bounded by the different the Red List thresholds), the majority of species have AOH that is as far from the threshold for downlisting as possible. In other words, most species are closer to the threshold for uplisting than for downlisting. We found that proximity to downlisting threshold did not differ between taxonomic classes (@STATS) and biogeographic realms (@STATS), but increased with increasing asbsolute AOH (@STATS). Thus, species are relatively further from the threshold for downlisting when absolute habitat quantity is lower.
...
```
The three realms with the most NES are Neotropical (1101 species), Afrotropical (571 species), and Indomalayan (452 species). These were also the three realms with the most ENES. Mapping total NES and ENES by country (Figure 3), we see that certain countries - such as Colombia, Peru, Mexico and Madagascar - harbour a striking proportion of the world's most habitat-limited species.
