...
nes <- nes %>% mutate(Category = str_replace(Category, "Critically Endangered", "10 km^2")) %>% 
  mutate(Category = str_replace(Category, "Endangered", "500 km^2")) %>% 
  mutate(Category = str_replace(Category, "Vulnerable", "2000 km^2"))

# relevel Categories
nes$Category <- factor(nes$Category, levels = c("10 km^2", "500 km^2", "2000 km^2"))

ggplot(nes, aes(x=className, fill=className)) + geom_bar() + facet_wrap(~Category) + ggtitle("Class and Threshold of NES") + theme(axis.text.x = element_blank()) + labs(fill="Class") + xlab(" ") + ylab("Count")
```
