...
Across class and Red List thresholds, there is a right skew towards a large difference between species' AOH and the downlisting threshold, particularly for those species with AOH between 10 km^2 and 500 km^2 (Fig. \@ref(fig:difference-histogram)). This suggests that, within each range of AOH values (bounded by the different the Red List thresholds), the majority of species have AOH that is as far from the threshold for downlisting as possible. In other words, most species are closer to the threshold for uplisting than for downlisting. We found that proximity to downlisting threshold did not differ between taxonomic classes (p-value = 0.988, 4 DF) and biogeographic realms (p-value = 0.999, 6 DF), but increased with increasing absolute AOH (p-value < 2.2e-16, 1 DF). Thus, species are relatively further from the threshold for downlisting when absolute habitat quantity is lower.
...
(ref:cap-2) Distribution of Differences to Next Threshold Across Class and Category. A histogram of NES within each class and AOH threshold met. Difference is 'proximity to threshold', calculated as the absolute difference between a species' AOH and the max threshold of its AOH category, divided by the maximum possible difference. 

```{r difference-histogram, fig.cap = "(ref:cap-2)"}
...
(ref:cap-3) a) The total number of threatened species within each country classified as 'No Exit Species' (NES). Results are summarized for each taxonomic group (rows), and shaded from low NES count (yellow) to high (red). b) The total number of threatened species within each country classified as 'Extreme No Exit Species' (ENES), those species with large differences between AOH and threshold. Results are summarized for each taxonomic group (rows), and shaded from low NES count (yellow) to high (red).

```{r map, fig.cap = "(ref:cap-3)"}
map <- 
