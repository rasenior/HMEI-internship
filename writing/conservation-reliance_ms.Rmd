...
# relevel Categories
nes$Category <- 
  factor(nes$Category, levels = c("AOH < 10 km^2", "AOH < 500 km^2", "AOH < 2000 km^2"))
nes$className <- 
  factor(nes$className, 
         levels = unique(nes$className),
         labels = capitalise(unique(nes$className)))
...
ggplot(nes, aes(x=className, fill=className)) + 
  geom_bar() + 
  facet_wrap(~Category, labeller = label_wrap_gen(width = 12, multi_line = TRUE)) + 
...
```{r map, eval = FALSE}
map <- 
  readPNG("HMEI-GDrive/figs/IUCN Data Maps/combi_crs_map_country.png") %>% 
